        ...
        for r in range(0, img_h, grid_h):
            for c in range(0, img_w, grid_w):
                ...
                if row_block == col_block:
                    array[r:min(r+grid_h, img_h), c:min(c+grid_w, img_w)] = 200 # Light gray
                else:
                     array[r:min(r+grid_h, img_h), c:min(c+grid_w, img_w)] = 50 # Dark gray

    # Add noise if requested
    if noise_level > 0:
        noise = np.random.randint(-noise_level, noise_level + 1, size=array.shape, dtype=np.int16)
...
def test_logo():
    ...
    assert detected_w >= w_min, f"Detected width {detected_w} is less than minimum {w_min}"
    assert detected_w <= w_max, f"Detected width {detected_w} is greater than maximum {w_max}"
    h = 1 + 3 + 1 + 6 + 1 + 5 + 1 + 2 + 1
