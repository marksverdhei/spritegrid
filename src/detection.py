...
def find_dominant_spacing(profile: np.ndarray, min_spacing: int = 3, prominence_ratio: float = 0.05) -> int:
    ...
    peaks, properties = find_peaks(profile, distance=min_spacing, prominence=min_prominence)
...
def detect_grid(image: Image.Image, min_grid_size: int = 4, smoothing_sigma: float = 1.0) -> Tuple[int, int]:
    """
    ...
    try:
        ...
        if img_h < actual_min_spacing * 2 or img_w < actual_min_spacing * 2:
             print(f"Error: Image dimensions ({img_w}x{img_h}) too small for analysis with min_spacing={actual_min_spacing}.", file=sys.stderr)
             return (0, 0)
        ...
        gradient_h_lines = np.abs(np.diff(img_array, axis=1, append=img_array[:, -1:])) # Keep same width
        # Horizontal edges (changes along columns) -> contribute to vertical profile
        gradient_v_lines = np.abs(np.diff(img_array, axis=0, append=img_array[-1:, :])) # Keep same height
        ...
        detected_grid_h = find_dominant_spacing(profile_v_smooth, min_spacing=actual_min_spacing)
        ...
        detected_grid_w = find_dominant_spacing(profile_h_smooth, min_spacing=actual_min_spacing)


        if detected_grid_w <= 0 or detected_grid_h <= 0:
             # find_dominant_spacing returns 0 on failure/insufficient peaks
             print(f"Warning: Failed to detect a reliable grid spacing (w={detected_grid_w}, h={detected_grid_h}).", file=sys.stderr)
        ...
        return detected_grid_w, detected_grid_h
         ...
         print("Error: SciPy or NumPy library not found. Please install them (`pip install numpy scipy pillow`).", file=sys.stderr)
         return (0, 0)
